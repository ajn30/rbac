...
if (isset($_GET['booking_id']) && is_numeric($_GET['booking_id'])) {
    $booking_id_to_cancel = $_GET['booking_id'];

    $stmt = $conn->prepare("DELETE FROM bookings WHERE booking_id = ?");
    $stmt->bind_param("i", $booking_id_to_cancel);

    if ($stmt->execute()) {
        log_activity($conn, $user_id, $role_id, 'User ' . $_SESSION['email'] . ' cancelled Booking ID: ' . $booking_id_to_cancel);
        $message = "Booking ID " . $booking_id_to_cancel . " cancelled successfully.";
        // Redirect back to view_booked_rooms after cancellation
        header("Location: view_booked_rooms.php");
        exit();
    } else {
...
// Fetch all bookings to display for selection (or for a list to click on)
$bookings = [];
                    ...
                    <table>
                        ...
                        <?php foreach ($bookings as $booking): ?>
                            <tr>
                                ...
                                <td><a href="cancel_booking.php?booking_id=<?= $booking['booking_id'] ?>" onclick="return confirm('Are you sure you want to cancel this booking?');">Cancel</a></td>
                            </tr>
                        <?php endforeach; ?>
                    </table>
                <?php else: ?>
